...
fn measure(
    ...
    for (0..reps) |_| {
        ...
        const hs = try myers.hunk.hunks(alloc, script.items, a.len, b.len, 3);
        defer alloc.free(hs);
...
fn measurePipeline(alloc: std.mem.Allocator, io: std.Io, a: []const u8, b: []const u8, reps: usize) !PipelineSample {
    ...
    for (0..reps) |_| {
        ...
        const old_t = try myers.token.tokenizeBy(alloc, a, '\n', &internMap);
        defer alloc.free(old_t.tokens);
        defer alloc.free(old_t.ids);

        const new_t = try myers.token.tokenizeBy(alloc, b, '\n', &internMap);
        ...
        const hs = try myers.hunk.hunks(alloc, script.items, old_t.tokens.len, new_t.tokens.len, 3);
        defer alloc.free(hs);
